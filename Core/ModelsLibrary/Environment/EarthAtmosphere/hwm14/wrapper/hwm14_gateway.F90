...
! 3. xyz = real(d_xyz) for real*4 or xyz = int(d_xyz) for integer*4 convert Fortran double to hwm14 expected data typ
  ...
  ! iyd (INTEGER*4)
   p_iyd = mxGetDoubles(prhs(1))
   call mxCopyPtrToReal8(p_iyd, d_iyd, 1)
   iyd = int(d_iyd)

  ! sec
   p_sec = mxGetDoubles(prhs(2))
