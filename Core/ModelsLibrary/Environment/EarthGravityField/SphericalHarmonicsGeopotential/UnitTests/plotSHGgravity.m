...
gravitationalField_max_Degree = 4;
...
for i=1:10
    ...
    [gx,gy,gz] = gravitysphericalharmonic( position_BI_I__m', 'egm96', gravitationalField_max_Degree,'Error' );
    acc_matlab_I__m_per_s2 = [gx,gy,gz];
    clearvars gx gy gz

    log_position(i,:) = position_BI_I__m;
    log_acceleration_shg(i,:) = gravitational_acceleration_I__m_per_s2;
...
title('Components of gravity vector')
xlabel('index')
ylabel('$e~(m/s^2)$',Interpreter='latex')
legend('a_{shg}','','','a_{matlab}')


% Error Components
subplot(3,1,2)
plot(log_acceleration_shg-log_acceleration_matlab)
title('Error in components of gravity vector')
xlabel('index')
ylabel('$e~(m/s^2)$',Interpreter='latex')
legend('a_{shg}-a_{matlab}','','')
...
title('Error in magnitude of gravity')
xlabel('index')
ylabel('$|e|~(m/s^2)$',Interpreter='latex')
legend('|a_{shg}|-|a_{matlab}|')
